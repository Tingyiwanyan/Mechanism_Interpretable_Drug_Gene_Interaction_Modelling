...
layer_one_hot = tf.keras.layers.CategoryEncoding(num_tokens=8, output_mode="one_hot")
input_gene_prior = []
input_gene_exp_one_hot = []
input_drug_one_hot = []
input_drug_response = []
input_rel_distance = []
input_smile_seq = []
input_cell_line_name = []
input_drug_name = []
input_interpret_smile = []
cell_line_name_train = cell_line_name_avail#[60:]
for i in cell_line_name_train:
    index = 0
    #print(i)
    for j in drug_names:
        print(j)
        #if i in leukemia_cell_lines and j=='nilotinib':
            #print("im in nilotinib Leukemia")
            #continue
        all_one_vector = np.zeros(6144)
        all_zero_vector = np.ones(6144)*-1e7
        if not np.isnan(cell_line_drug.loc[i][j]):
            try:
                ...
                if indexes == []:
                    #print(j)
                    input_gene_prior.append(all_one_vector)
                else:
                    for each_gene_index in indexes:
                        all_zero_vector[each_gene_index] = 0
                    input_gene_prior.append(all_zero_vector)
            except:
                input_gene_prior.append(all_one_vector)
            drug_smile_single = df_drug_smile.loc[j][0]#CCLE_drug_smiles[index]
            #print(drug_smile_single)
            rel_distance_ = generate_rel_dist_matrix(drug_smile_single)
            interpret_smile = generate_interpret_smile(drug_smile_single)[0]
            input_interpret_smile.append(interpret_smile)
            input_drug_name.append(j)
            #input_rel_distance.append(rel_distance_)
            input_smile_seq.append(drug_smile_single)
            input_drug_response.append(cell_line_drug.loc[i][j])
            input_cell_line_name.append(i)
        index+=1



cell_line_name_val = cell_line_name_avail[0:10]
...
batch_size=30
train_data = tf.data.Dataset.from_tensor_slices((np.array(input_drug_name), np.array(input_smile_seq), np.array(input_interpret_smile),
                                                 np.array(input_cell_line_name), np.array(input_drug_response), np.array(input_gene_prior)))
...
for epoch in range(3):
    ...
    for step, (batch_drug_name, batch_smile_seq, 
        ...
        decode_batch_smile_seq = [x.decode('utf-8') for x in list(np.array(batch_smile_seq))]
        decode_batch_interpret_smile = [x.decode('utf-8') for x in list(np.array(batch_interpret_smile))]
        ...
        with tf.GradientTape() as tape:
            ...
            index_score = tf.math.add(tf.cast(batch_gene_prior, tf.float32), score_cross_global[:,0,:])
            nominator = tf.reduce_sum(tf.math.exp(index_score),axis=-1)
            denominator = tf.reduce_sum(tf.math.exp(score_cross_global[:,0,:]),axis=-1)
            loss_contrast = tf.reduce_mean(tf.math.log(tf.math.divide(nominator, denominator)),0)
    
            
            identity_drug = tf.eye(batch_shape)        
            X_global_flatten = tf.reshape(X_global_, shape=(batch_shape,30))
            X_global_score = tf.math.exp(tf.matmul(X_global_flatten,X_global_flatten,transpose_b=True))
            X_global_score_nominator = tf.reduce_sum(tf.multiply(identity_drug, X_global_score),axis=-1)
            X_global_score_denominator = tf.reduce_sum(X_global_score,axis=-1)
            #loss_drug = tf.reduce_mean(tf.reduce_mean(X_global_score,axis=-1),axis=-1)
        ...
        optimizer.apply_gradients(zip(gradients, model_midi.trainable_variables))
        #print(prediction)
        #print(batch_drug_respons)
        ...
        if step % 20 == 0:
            ...
            prediction_val = model_midi((drug_atom_one_hot_chunk_val, gene_expression_chunk_val, 
                                         drug_smile_length_chunk_val, drug_rel_position_chunk_val, 
                                         edge_type_matrix_chunk_val, gene_mutation_bin_chunk_val,mask_val))[0][:,0]
            #prediction_val = model_midi((drug_atom_one_hot_chunk_val, gene_expression_chunk_val,drug_smile_length_chunk_val))[:,0]
            #print(prediction_val)
            #print(np.array(input_drug_response_val))
